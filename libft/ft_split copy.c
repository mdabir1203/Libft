...
/*   Updated: 2022/06/04 17:40:02 by mabbas           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**ft_split(char const *str, char d)
{
	...
	result_arr = (char **) malloc((ft_sen_len(str, d) + 1) * sizeof(char *));
	if (!result_arr)
		return (NULL);
...
{ 
	...
	resut = ft_split("_UNasda_ DEUasdaX TROIS", '_');
	
	i = 0;
	while (i < 3)
	{
		printf("Split array output: %s\n",resut[i]);
		i++;
	}
